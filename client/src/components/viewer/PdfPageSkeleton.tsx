    ...
    <div className="flex w-full justify-center px-2 py-2 md:px-6 md:py-4">
      <div
        className="
          ...
          bg-white
          shadow-[0_20px_50px_-12px_rgba(15,23,42,0.18),0_8px_20px_-8px_rgba(15,23,42,0.10)]
          dark:border-white/[0.09]
          dark:bg-white/[0.045]
          dark:shadow-[0_24px_60px_-16px_rgba(0,0,0,0.55),0_10px_24px_-10px_rgba(0,0,0,0.4)]
        "
      ...
      >
        ...
        <div
          ...
          style={{
            background:
              'linear-gradient(105deg, transparent 32%, rgba(148,163,184,0.14) 50%, transparent 68%)',
        ...
        <div style={{ padding: p }}>
          ...
          <div style={{ marginBottom: bg }}>
            <div
              className="ppsk-pulse rounded-md bg-slate-200 dark:bg-white/[0.11]"
              style={{ height: Math.round(lh * 2.1), width: '62%', marginBottom: Math.round(lg * 1.4) }}
            />
            <div
              className="ppsk-pulse rounded bg-slate-200/80 dark:bg-white/[0.09]"
              style={{ height: Math.round(lh * 1.1), width: '38%', marginBottom: Math.round(bg * 0.9), animationDelay: '0.1s' }}
            />
            <div
              className="rounded-full bg-gradient-to-r from-transparent via-slate-300/70 to-transparent dark:via-white/[0.12]"
          ...
          <div style={{ marginBottom: bg }}>
            {[96, 90, 94, 92, 87, 91, 44].map((pct, i) => (
              <div
                key={i}
                className="ppsk-pulse rounded-full bg-slate-200/90 dark:bg-white/[0.10]"
                style={{
                  height: lh,
                  width: `${pct}%`,
            ...
            {[[90, 94, 86, 92, 54], [93, 87, 95, 89, 48]].map((lines, ci) => (
              <div key={ci} style={{ flex: 1 }}>
                {lines.map((pct, li) => (
                  <div
                    key={li}
                    className="ppsk-pulse rounded-full bg-slate-200/80 dark:bg-white/[0.09]"
                    style={{
                      height: lh,
                      width: `${pct}%`,
          ...
          {/* Figure */}
          <div
            className="ppsk-pulse relative mx-auto overflow-hidden rounded-xl border border-slate-200/70 bg-slate-100/70 dark:border-white/[0.08] dark:bg-white/[0.05]"
            style={{
          ...
          >
            <div className="flex h-full items-center justify-center">
              <svg
                aria-hidden="true"
                className="text-slate-300 dark:text-white/[0.14]"
          ...
          <div style={{ marginBottom: bg }}>
            {[95, 89, 93, 87, 91, 84, 58].map((pct, i) => (
              <div
                key={i}
                className="ppsk-pulse rounded-full bg-slate-200/75 dark:bg-white/[0.085]"
                style={{
                  height: lh,
                  width: `${pct}%`,
                  marginBottom: i < 6 ? lg : 0,
          ...
          {/* Table block */}
          <div
            className="ppsk-pulse relative mx-auto overflow-hidden rounded-xl border border-slate-200/70 bg-slate-100/60 dark:border-white/[0.08] dark:bg-white/[0.045]"
            style={{
            ...
            <div className="flex h-full flex-col justify-around p-3">
              {[0, 1, 2, 3].map((i) => (
                <div key={i} className="flex gap-3">
                  <div className="h-2 flex-1 rounded-full bg-slate-300/60 dark:bg-white/[0.10]" />
                  <div className="h-2 flex-1 rounded-full bg-slate-300/50 dark:bg-white/[0.08]" />
                  <div className="h-2 flex-1 rounded-full bg-slate-300/60 dark:bg-white/[0.10]" />
                  <div className="h-2 flex-1 rounded-full bg-slate-300/50 dark:bg-white/[0.08]" />
                </div>
              ))}
        ...
        <div className="absolute inset-x-0 bottom-4 flex items-center justify-center">
          <div className="ppsk-pulse flex items-center gap-1.5 rounded-full bg-slate-100 px-3 py-1 dark:bg-white/[0.07]">
            <div className="rounded-full bg-slate-300 dark:bg-white/[0.18]" style={{ height: 6, width: 6 }} />
            <div className="rounded-full bg-slate-300 dark:bg-white/[0.18]" style={{ height: 8, width: 24 }} />
          </div>
        </div>
      </div>
    </div>
